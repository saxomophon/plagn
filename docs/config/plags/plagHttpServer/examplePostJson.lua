...
local httpFormDataTable = httpContentUtils.parse_json(reqContent)
...
-- set up the resp table and encode it to json
local respTable = {}
respTable["status"] = "ok"
respContent = httpContentUtils.parse_table(respTable)

-- set the content type to x-www-form-urlencoded
respHeader["Content-Type"] = httpContentTypes.get("json")
